...
export default function Navbar() {
  ...
  // Check if navbar should have solid white background (Always solid on subpages, solid on scroll for Home)
          ...
          <Link href="/" className="flex items-center space-x-2 group">
            <Compass
              className={`h-8 w-8 transition-transform duration-500 group-hover:rotate-180 ${
                isSolid ? "text-blue-600" : "text-emerald-400"
              }`}
            />
            <div className="flex flex-col">
              ...
              <span className={`text-[10px] uppercase tracking-[0.2em] font-semibold -mt-1.5 ${
                isSolid ? "text-emerald-600" : "text-emerald-400"
              }`}>
                Travels
              </span>
            </div>
      ...
      >
        <div className="px-4 space-y-1 sm:px-3 text-slate-900">
          ...
          <div className="pt-4 border-t border-slate-100 px-3 flex flex-col space-y-3">
            <a
              href="tel:9829135440"
              className="flex items-center justify-center space-x-2 text-slate-700 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold"
            >
            ...
            <Link
              href="/contact?book=true"
              onClick={() => setIsOpen(false)}
              className="block text-center bg-blue-600 text-white py-3 rounded-xl text-sm font-bold shadow-md hover:bg-blue-700"
            >
              Book Now
            </Link>
          </div>
